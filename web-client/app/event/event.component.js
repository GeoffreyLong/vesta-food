angular.module('event').component('event', {
  ...
  controller: function EventController($scope, $http, $location, $window,
                                        authService){
    if (!$scope.event){
      $http.get('/api' + $location.path()).then(function(event) {
    ...
    $scope.slickStyle = function() {
      if ($scope.foodLoaded && $scope.event.foods.length <= 3) {
